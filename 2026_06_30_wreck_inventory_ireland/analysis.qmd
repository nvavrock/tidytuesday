...
Charts below embed copies of the PNGs in `output/` (same files used for LinkedIn and slides).
...
```{r location}
knitr::include_graphics("report-figures/01_location_status.png")
```
...
```{r time}
knitr::include_graphics("report-figures/02_wrecks_over_time.png")
```
...
```{r map}
knitr::include_graphics("report-figures/03_wreck_map.png")
```

**Takeaway:** Mapped wrecks cluster along **Ireland's coasts, shipping lanes, and inland waters** (loughs and rivers where coordinates exist). The basemap uses Tailte Éireann and OSNI land outlines — Irish tricolor on the Republic, St Patrick's saltire on Northern Ireland — with major lakes from Natural Earth 1:10m. Dublin and Cork are marked in red and blue. Only ~3,500 records (~20%) have coordinates.
...
```{r class}
knitr::include_graphics("report-figures/04_top_classifications.png")
```
